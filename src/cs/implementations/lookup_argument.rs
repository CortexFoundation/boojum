...
> {
    ...
    for (path_element, src) in lookup_selector_path[1..]
        .iter()
    ...
    for (beta, gamma) in betas.iter().zip(gammas.iter()) {
        ...
        let mut aggregated_lookup_columns =
        ...
        worker.scope(domain_size / P::SIZE_FACTOR, |scope, chunk_size| {
            ...
            for (idx, _) in aggregated_lookup_columns.spare_capacity_mut()
            ...
            assert_eq!(
                aggregated_lookup_columns.spare_capacity_mut()[..domain_size / P::SIZE_FACTOR]
                    .chunks_mut(chunk_size)
                    .len(),
                subiterators.len()
            );
            for (dst, src) in aggregated_lookup_columns.spare_capacity_mut()
                [..domain_size / P::SIZE_FACTOR]
                .chunks_mut(chunk_size)
                    ...
                    for dst in dst.iter_mut() {
                        ...
                        for (src, gamma) in src.iter_mut().zip(powers_of_gamma.iter()) {
                            P::mul_and_accumulate_into(
                                &mut acc,
                                src.next().expect("table column element"),
                                gamma,
                                &mut ctx,
                            );
                        }
        ...
        let mut aggregated_lookup_columns_inversed =
            P::vec_into_base_vec(aggregated_lookup_columns);
        ...
        for (witness_columns, multiplicity_column) in variables_columns
            .chunks_exact(column_elements_per_subargument)
            .zip(multiplicities_columns.iter())
        {
            ...
            worker.scope(domain_size / P::SIZE_FACTOR, |scope, chunk_size| {
                ...
                assert_eq!(
                    a_poly.spare_capacity_mut()[..domain_size / P::SIZE_FACTOR]
                        .chunks_mut(chunk_size)
                        .len(),
                    subiterators.len()
                );

                for (dst_a, src) in a_poly.spare_capacity_mut()[..domain_size / P::SIZE_FACTOR]
                    .chunks_mut(chunk_size)
                    .zip(subiterators.into_iter())
                {
                    let powers_of_gamma = &powers_of_gamma[..];
                    ...
                    scope.spawn(move |_| {
                        ...
                        for dst_a in dst_a.iter_mut() {
                            ...
                            for (src, gamma) in src.iter_mut().zip(powers_of_gamma.iter()) {
                                P::mul_and_accumulate_into(
                                    &mut acc,
                                    src.next().expect("witness column element"),
                                    gamma,
                                    &mut ctx,
                                );
                            }
            ...
            worker.scope(a_poly.len(), |scope, chunk_size| {
                for (((dst_a, dst_b), mults), selector) in a_poly
                    .chunks_mut(chunk_size)
    ...
    for (beta, gamma) in betas.iter().zip(gammas.iter()) {
            ...
            variables_columns_for_lookup.chunks_exact(num_variable_columns_per_argument)
        {
            ...
            worker.scope(domain_size / P::SIZE_FACTOR, |scope, chunk_size| {
                ...
                assert_eq!(
                    witness_encoding_poly.spare_capacity_mut()[..domain_size / P::SIZE_FACTOR]
                        .chunks_mut(chunk_size)
                        .len(),
                    subiterators.len()
                );

                for (dst_a, src) in witness_encoding_poly.spare_capacity_mut()
                    [..domain_size / P::SIZE_FACTOR]
                    .chunks_mut(chunk_size)
                    .zip(subiterators.into_iter())
                {
            ...
            unsafe { witness_encoding_poly.set_len(domain_size / P::SIZE_FACTOR) };
            let mut witness_encoding_poly = P::vec_into_base_vec(witness_encoding_poly);
            batch_inverse_inplace_parallel::<F, A>(&mut witness_encoding_poly, worker);
            let witness_encoding_poly = P::vec_from_base_vec(witness_encoding_poly);

            // push the results
            let witness_encoding_poly = GenericPolynomial::from_storage(witness_encoding_poly);

            subarguments_witness_encoding_polys.push(witness_encoding_poly);
        }
        ...
        for multiplicity_column in multiplicities_columns.iter() {
            ...
            let mut multiplicities_encoding_poly = aggregated_lookup_columns_inversed.clone();

            worker.scope(multiplicities_encoding_poly.len(), |scope, chunk_size| {
                for (dst_b, mults) in multiplicities_encoding_poly
                    .chunks_mut(chunk_size)
                    .zip(multiplicity_column.storage.chunks(chunk_size))
                {
                    let mut ctx = *ctx;
            ...
            let multiplicities_encoding_poly =
                GenericPolynomial::from_storage(multiplicities_encoding_poly);

            subarguments_multiplicities_encoding_polys.push(multiplicities_encoding_poly);
        }
        ...
        if crate::config::DEBUG_SATISFIABLE == true {
            ...
            for a_poly in subarguments_witness_encoding_polys.iter() {
                for a in P::slice_into_base_slice(&a_poly.storage).iter() {
                    a_sum.add_assign(a);
                }
            }
            ...
            for b_poly in subarguments_multiplicities_encoding_polys.iter() {
                for b in P::slice_into_base_slice(&b_poly.storage).iter() {
                    b_sum.add_assign(b);
                }
            }
            ...
            if a_sum != b_sum {
                panic!(
                    "Sumcheck fails with a = {:?}, b = {:?} in arg repetition {}",
                    a_sum,
                    b_sum,
                    result.len()
                );
